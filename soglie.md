...
def generate_sample_data():
    t = np.linspace(0, 2, 200)  # 2 secondi, 100Hz
    static_weight = 700  # N
    noise = np.random.normal(0, 10, len(t))
    
    # Simula un salto
    force = static_weight * np.ones_like(t)
    force[40:60] += 300 * (t[40:60] - t[40])  # Caricamento
    force[60:80] = 0  # Volo
    force[80:100] = static_weight + 500 * np.exp(-(t[80:100] - t[80])*10)  # Atterraggio
...
def derivative_method(t, force, threshold=1000):
    ...
    # Trova eventi usando la derivata
...
def statistical_method(force, static_weight):
    mean = static_weight
    std = np.std(force[:100])  # Primi 100 campioni per rumore base
...
def ml_method(force, train=True):
    ...
    if train:
        # Simula labels per training (in pratica usare dati etichettati reali)
        labels = np.zeros(len(features))
        labels[40:60] = 1  # start
        labels[60:80] = 2  # flight
        labels[80:100] = 3  # landing
